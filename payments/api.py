...
def transaction_update_status(request, txn_id):
    txn = get_object_or_404(Transaction, id=txn_id)
...
# List manual payment numbers
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def payment_numbers(request):
...
def manual_payment_submit(request, txn_id):
    ...
    txn.status = 'pending'
    txn.save()
    return Response({'detail': 'Manual payment submitted', 'txn_id': txn.id})
...
# Bkash/Nagad payment callback
@api_view(['POST'])
@permission_classes([])
def manual_gateway_callback(request, txn_id):
    txn = get_object_or_404(Transaction, id=txn_id)
    data = request.data
